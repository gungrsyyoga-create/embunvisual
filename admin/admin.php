...
<body>
    ...
    <aside class="sidebar" data-aos="fade-right" data-aos-duration="800">
        ...
        <a href="?menu=folder_manager" class="nav-item <?= $menu == 'folder_manager' ? 'active' : '' ?>" style="background:rgba(255,193,7,0.1); color:#FFC107;">
            <i class="fas fa-folder-open"></i> Kelola Folder Klien
        </a>
                ...
                <form method="POST" action="admin.php?menu=admin" style="padding: 25px;">
                    ...
                    <div class="form-group">
                        ...
                        <select name="role_baru" class="form-control" required>
                            <option value="Basic">👤 Basic Staff (Monitor Undangan Basic Tier)</option>
                            <option value="Staff">👨‍💻 Staff (Kerjakan Semua Penugasan)</option>
                            <option value="Super Admin">👑 Super Admin (Akses Penuh Seluruh Sistem)</option>
                        </select>
                        <span class="text-hint">
                            • Basic: Hanya lihat & pantau undangan tier Basic<br>
                            • Staff: Akses penuh untuk mengerjakan pesanan yang ditugaskan<br>
...
Merupakan suatu kehormatan apabila Anda berkenan hadir. Terima kasih!</textarea>
        ...
        <?php } elseif($menu == 'folder_manager') { 
            // Super Admin: Manage Folder Client
            if($current_role != 'Super Admin') {
                $_SESSION['notif_pesan'] = "Swal.fire('Akses Ditolak!', 'Hanya Super Admin yang dapat mengelola folder klien.', 'error');";
                header("Location: admin.php?menu=dashboard"); exit;
            }
            
            // Ambil daftar pesanan yang sudah punya akun premium
            $q_folder = mysqli_query($conn, "SELECT p.*, k.nama_tema, kp.id as kp_id, kp.tipe, kp.folder_path, kp.is_active 
                FROM pesanan p 
                LEFT JOIN katalog_tema k ON p.tema_id=k.id 
            ...
            // Handle generate/update folder
            if(isset($_POST['action']) && $_POST['action'] == 'generate_folder_new') {
        ...
        <div class="page-header" data-aos="fade-down">
            <h1 class="page-title">🗂️ Kelola Folder Klien</h1>
            <p style="color:var(--muted); margin-top:5px;">Buat dan kelola folder undangan untuk setiap klien berdasarkan paket tier mereka.</p>
        </div>
        
        <div class="card" data-aos="fade-up" data-aos-delay="200">
            <div class="card-header"><i class="fas fa-list"></i> Daftar Klien & Folder</div>
            <div style="overflow-x: auto;">
                <table id="dataTable">
                    <thead>
                        <tr>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>
                                ...
                                <span style="font-size:0.85rem; color:var(--muted);"><?= htmlspecialchars($f['nama_pemesan']) ?></span>
                            </td>
                            <td><?= htmlspecialchars($f['nama_tema'] ?? '-') ?></td>
                            <td>
                                <?php if($f['tipe']) { ?>
                                    <span class="badge" style="<?php 
                                        if($f['tipe'] == 'basic') echo 'background:#64748b; color:white;';
                                        elseif($f['tipe'] == 'premium') echo 'background:var(--primary); color:white;';
                                        else echo 'background:linear-gradient(135deg, #D4AF37, #B8960C); color:#fff;';
                                    ?>">
                                        <?= ucfirst($f['tipe']) ?>
                                    </span>
                                <?php } else { ?>
                                    <span style="color:var(--muted); font-size:0.85rem;">Belum ditentukan</span>
                                <?php } ?>
                            </td>
                            <td>
                                <?php if($f['folder_path']) { ?>
                                    <code style="background:#f0f0f0; padding:4px 8px; border-radius:4px; font-size:0.75rem;">
                                        <?= htmlspecialchars($f['folder_path']) ?>
                                    </code>
                                <?php } else { ?>
                                    <span style="color:var(--muted); font-size:0.8rem; font-style:italic;">📁 Belum ada</span>
                                <?php } ?>
                            </td>
                            <td>
                                <span class="badge" style="<?= $f['is_active'] ? 'background:#dcfce7; color:#166534;' : 'background:#fee2e2; color:#991b1b;' ?>">
                            ...
                            <td>
                                <div style="display:flex; gap:8px; flex-wrap:wrap;">
                                    <?php if(!$f['folder_path']) { ?>
                                        <!-- Jika belum ada folder, tampilkan dropdown + tombol -->
                                        <form method="POST" action="admin.php?menu=folder_manager" style="display:inline; margin:0;">
                                            <input type="hidden" name="action" value="generate_folder_new">
                                            <input type="hidden" name="pesanan_id" value="<?= $f['id'] ?>">
                                            <select name="tier" class="form-control" style="padding:6px; font-size:0.8rem; width:120px; display:inline-block; margin-right:6px;" required>
                                                <option value="">Pilih Tier</option>
                                                <option value="basic">Basic</option>
                                                <option value="premium">Premium</option>
                                                <option value="exclusive">Exclusive</option>
                                            </select>
                                            <button type="submit" class="btn-action btn-primary" style="padding:6px 12px; font-size:0.8rem;">
                                                <i class="fas fa-folder-plus"></i> Buat Folder
                                            </button>
                                        </form>
                                    <?php } else { ?>
                                        <!-- Jika sudah ada folder -->
                                        <a href="/embunvisual/<?= $f['folder_path'] ?>" target="_blank" class="btn-action btn-secondary" style="padding:6px 12px; font-size:0.8rem; text-decoration:none;">
                                            <i class="fas fa-external-link-alt"></i> Buka Folder
                                        </a>
                                        <button type="button" onclick="alert('Folder sudah aktif. Edit dilakukan via form di atas.')" class="btn-action btn-secondary" style="padding:6px 12px; font-size:0.8rem;">
